...
export default function Sidebar() {
  ...
  const nav = [
    { href: '/',              icon: LayoutDashboard, label: 'Dashboard',     vi: 'Tổng quan',  show: true },
    { href: '/events',        icon: CalendarDays,    label: 'Events',        vi: 'Sự kiện',    show: true },
    { href: '/tasks',         icon: CheckSquare,     label: 'Tasks',         vi: 'Công việc',  show: true },
    { href: '/notifications', icon: Bell,            label: 'Notifications', vi: 'Thông báo',  show: true },
    { href: '/ai',            icon: Bot,             label: 'AI Assistant',  vi: 'Trợ lý AI',  show: isManager },
    { href: '/users',         icon: Users,           label: 'Team',          vi: 'Nhân viên',  show: isManager },
  ]
  ...
  return (
    <aside style={{
      ...
      <nav style={{ flex: 1, padding: '12px 10px' }}>
        ...
        {nav.filter(n => n.show).map(({ href, icon: Icon, label, vi }) => {
      ...
      <div style={{ padding: '12px 10px', borderTop: '1px solid var(--border)' }}>
        ...
        <div
          ...
          <p style={{ fontSize: '13px', color: 'var(--accent-red)' }}>Sign Out / Đăng xuất</p>
        </div>
      </div>
    </aside>
  )
