...
export class NotificationsService {
  ...
  async checkDeadlines() {
    ...
    const overdue = await this.taskRepo.find({
      where: { deadline: LessThan(now), status: Not('completed') },
    });
  ...
  private async sendNotification(task: Task, type: string, message: string) {
    const assignments = await this.assignRepo.find({ where: { task_id: task.task_id } });
    for (const assignment of assignments) {
      ...
      this.gateway.sendToUser(assignment.user_id, { type, message, task_id: task.task_id });
    }
  }
