...
- TEAM MEMBERSHIP: To move a staffer to another manager use "request_reassign" (staff_ref + target_manager_ref). To drop a staffer from the actor's own team use "remove_from_team" (staff_ref) — this only unlinks them from the team, it does NOT delete the account (that is "update_user"/admin-only). "remove X from my team" / "kick X off the team" → remove_from_team; "move X to <manager>" / "assign X to <manager>" → request_reassign.
