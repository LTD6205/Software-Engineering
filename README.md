...
- **AI commands** — managers and organizers describe changes in plain English/Vietnamese; the model turns them into real operations — creating/updating/assigning/grouping/deleting tasks, undo, and (role permitting) event and account changes. Runs in **Auto-accept** or **Ask-first** mode (preview, then confirm/cancel). Times are interpreted in Vietnam time (UTC+7).
